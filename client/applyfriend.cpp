...
void ApplyFriend::InitTipLbs()
{
    ...
    for(int i = 0; i < _tip_data.size(); i++){
        ...
        connect(lb, &ClickedLable::clicked, this, &ApplyFriend::SlotChangeFriendLabelByTip);
...
void ApplyFriend::AddTipLbs(ClickedLable* lb, QPoint cur_point, QPoint& next_point, int text_width, int text_height)
{
	lb->move(cur_point);
	lb->show();
...
void ApplyFriend::SetSearchInfo(std::shared_ptr<SearchInfo> si)
{
    ...
    qDebug() << "applyname is :" << applyname << " bakname is : ";
